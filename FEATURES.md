...
| App Store | GDPR privacy webhook handlers | Done | P0 | Unified handler for `customers/data_request`, `customers/redact`, and `shop/redact` in `app/routes/webhooks.privacy.tsx` & subscribed with `compliance_topics` in `shopify.app.toml`. |
